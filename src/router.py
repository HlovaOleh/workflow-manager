...
from src.models import DBNode, DBWorkflow
...
async def delete_workflow(workflow_id: int, db: Session = Depends(get_db)) -> DBWorkflow:
...
async def create_node(
    ...
    """Create a new node (Only for creating Start and End nodes)"""
    return crud.create_node(db=db, node=node)


@router.get("/nodes/", response_model=list[Node])
async def read_all_nodes(db: Session = Depends(get_db)) -> list[DBNode]:
    """Get all nodes"""
    return crud.get_all_nodes(db=db)


@router.get("/nodes/{node_id}/", response_model=Node)
async def read_node(node_id: int, db: Session = Depends(get_db)) -> DBNode:
...
async def delete_node(node_id: int, db: Session = Depends(get_db)) -> DBNode:
...
async def update_message_node(
        node_id: int, data: MessageNodeCreate,
        db: Session = Depends(get_db)
) -> DBNode:
...
async def update_condition_node(
        node_id: int, data: ConditionNodeCreate,
        db: Session = Depends(get_db)
) -> DBNode:
...
async def execute_workflow(workflow_id: int, db: Session = Depends(get_db)) -> list:
    """Creates a new graph,
    adds all the nodes and edges to the graph
    and looking for the shortest path"""
